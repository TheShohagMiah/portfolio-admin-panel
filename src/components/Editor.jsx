...
import { Color } from "@tiptap/extension-color"; // ✅ named
import { TextStyle } from "@tiptap/extension-text-style"; // ✅ named
...
//  - onChange    (fn)       called with HTML string on every update
//  - hasError    (bool)     shows red border if true
//  - placeholder (string)   placeholder text (optional)
//  - minHeight   (string)   min height of editor (default "180px")
//  - maxChars    (number)   max character count (optional)
// ═══════════════════════════════════════════════════════════════
  ...
  const editor = useEditor({
    extensions: [
      ...
      Placeholder.configure({
        placeholder,
        emptyEditorClass:
          "before:content-[attr(data-placeholder)] before:text-muted-foreground/40 before:float-left before:pointer-events-none before:h-0",
      }),
    ],
    content: value || "",
    onUpdate: ({ editor }) => {
      ...
      <div className="px-4 py-2 border-t border-border bg-secondary/30 flex items-center justify-between flex-wrap gap-2">
        <div className="flex items-center gap-3 flex-wrap">
          <span className="text-[10px] text-muted-foreground/40 font-mono">
            <kbd className="px-1.5 py-0.5 rounded bg-muted border border-border text-[9px]">
              Enter
            </kbd>{" "}
            paragraph
          </span>
        ...
        <div className="flex items-center gap-2">
          ...
          {maxChars && (
            <span
              className={`text-[10px] font-mono ${isOverLimit ? "text-destructive" : "text-muted-foreground/40"}`}
            >
              {charCount}/{maxChars}
            </span>
          )}
